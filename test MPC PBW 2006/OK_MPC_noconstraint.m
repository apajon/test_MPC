...
for i=1:round(max(phase_duration_cumul)/T)
    ...
    % Cost
    H=1/2*Q*z2_H+1/2*R*dddx2_H;
    f=1/2*Q*z2_f+1/2*R*dddx2_f;
...
clf
hold on
plot(T:T:phase_duration_cumul(end),z_ref_max,':k','LineWidth',2)
plot(T:T:phase_duration_cumul(end),z_ref_min,':k','LineWidth',2)
