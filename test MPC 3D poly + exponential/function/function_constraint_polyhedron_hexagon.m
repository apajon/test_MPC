function [A,b]=function_constraint_polyhedron_hexagon(...
    Pu_c,Pu_step,...
    xf_c,xf_step,...
    yf_c,yf_step,...
    zf_c,zf_step,...%zzmp_ref_reduce
    plan_hexagon,height_min)
    ...
    for i=1:size(plan_hexagon,2)
        for j=1:size(plan_hexagon{i},1)
            ...
            b_diff_c_p=[b_diff_c_p;...
                -plan_hexagon{i}(j,1)*xf_diff_c_p-plan_hexagon{i}(j,2)*yf_diff_c_p-plan_hexagon{i}(j,3)*zf_diff_c_p-plan_hexagon{i}(j,4)];
        end
    end
